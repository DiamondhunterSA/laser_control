...
namespace ImageAcquisition
{
    public partial class ImageAcquisition : Form
    {
        ...
        public ImageAcquisition()
        {
			...
			// false: user dvpOpenByName open the camera
			// true : user dvpOpenByUserId open the camera
        ...
        private void StartPlay_Click(object sender, EventArgs e)
        {
			...
			// init display count is 0
			m_dfDisplayCount = 0;
            if (IsValidHandle(m_handle))//相机已经打开成功
            {
                dvpStreamState state = dvpStreamState.STATE_STOPED;
                ...
                status = DVPCamera.dvpGetStreamState(m_handle, ref state);
				Debug.Assert(status == dvpStatus.DVP_STATUS_OK);
                if (state != dvpStreamState.STATE_STOPED)//视频流已经打开，点击关闭
                {
                    
                    //停止不执行，其他都执行
                    if ((m_pThread.ThreadState & System.Threading.ThreadState.Stopped) == 0)//if里面除了线程状态Stopped是1，其他都是0
                    {
                        m_ThreadEvent.Set();
                        m_pThread.Abort();
                    }
        ...
        public void ImageThread()
        {
            ...
            //ref dvpFrame pTargetFrame, uint targetBufferSize, dvpStreamFormat
            //uint targetBufferSize = 37748736;
            //dvpFrame pTargetFrame = new dvpFrame();
            //IntPtr pTargetBuffer = new IntPtr();
            while (!m_ThreadEvent.WaitOne(time_out))
            {
		        ...
		        if ( Display.Checked )
		        {
					...
					if (bDisplay)
					{
                        ...
                        status = DVPCamera.dvpDrawPicture(ref frame, pBuffer, m_ptr_wnd, (IntPtr)0, (IntPtr)0);
                        Debug.Assert(status == dvpStatus.DVP_STATUS_OK);
					}
		        }
